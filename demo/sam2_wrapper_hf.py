...
def load_sam2_tracker_hf(checkpoint_path=None, device="cuda"):
    """
    ...
    # Use provided path or default, ensuring it's an absolute path
    if checkpoint_path is None:
        # Path from download script is 'checkpoints/sam2/sam2.1_hiera_large.pt'
        default_path = os.path.join(script_dir, "checkpoints", "sam2", "sam2.1_hiera_large.pt")
        if os.path.exists(default_path):
            checkpoint_path = default_path
        else:
            # Fallback for other possible names/locations
            checkpoint_path = os.path.join(script_dir, "checkpoints", "sam2.1_hiera_large.pt")

    # Config file is local to the script/demo directory
